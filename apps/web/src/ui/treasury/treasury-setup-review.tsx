    ...
    >
      <div ref={headingRef} className="space-y-8">
        ...
        <div className="border-y border-line py-6">
          ...
          <p className="mt-5 text-sm font-semibold">Payment operators can execute approved Basin payments.</p>
          <p className="mt-5 text-sm font-semibold">Receiving details remain controlled by each payee.</p>
        </div>
        <dl className="grid grid-cols-[auto_1fr] gap-x-6 gap-y-3 text-sm">
          ...
          <dd className="text-right">{routineLimit ? "Basin payments only" : "Enabled when Basin payments are deployed"}</dd>
          {routineLimit ? <><dt className="text-ink-tertiary">Routine limit</dt><dd className="text-right font-mono text-xs">{routineLimit} USDC base units</dd></> : null}
        </dl>
        <p className="text-sm leading-relaxed text-ink-secondary">
          {routineLimit
            ? "Routine payment access is limited to the reviewed Basin payment action."
